...
export default function DAO(){
  ...
  const createProposal = async () => {
    try {
      console.log(votingProposal)
      const signer = await getProviderOrSigner(true);
      const daoContract = getDaoContractInstance(signer);
      const txn = await daoContract.createProposal(votingProposal);
      setLoading(true);
      await txn.wait();
  ...
  const fetchProposalById = async (id) => {
    try {
      ...
      const parsedProposal = {
        proposalId: id,
        votingProposal: proposal.votingProposal.toString(),
        deadline: new Date(parseInt(proposal.deadline.toString()) * 1000),
        yayVotes: proposal.yayVotes.toString(),
        nayVotes: proposal.nayVotes.toString(),
        voteFinalized: proposal.voteFinalized,
      };
      ...
      return (
        <div className={styles.container}>
          ...
          <textarea
            ...
            type="text"
            onChange={(e) => setVotingProposal(e.target.value)}
          />
        ...
        <div>
          {proposals.map((p, index) => (
            <div key={index} className={styles.proposalCard}>
              ...
              <p>Proposal: {p.votingProposal}</p>
              <p>Deadline: {p.deadline.toLocaleString()}</p>
              <p>Yay Votes: {p.yayVotes}</p>
              <p>Nay Votes: {p.nayVotes}</p>
              <p>Finalized?: {p.voteFinalized.toString()}</p>
              {p.deadline.getTime() > Date.now() && !p.voteFinalized ? (
                ...
                <div className={styles.description}>Vote Finalized</div>
              )}
            </div>
          ))}
        </div>
  ...
  return (
    <div>
      <Head>
        ...
        <meta name="description" content="Block the vote DAO" />
        <link rel="icon" href="/vote-nav.svg" />
      </Head>

      <div className={styles.main}>
        <div>
          <h1 className={styles.title}>Voting Proposals</h1>
